...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily\color{accent} sotOS Architecture}
...
sotOS is neither. It is a \textbf{verified microkernel with selective exokernel bypass}.
...
sotOS implements four layers of defense. Each layer catches what the layers
above it miss:
...
didn't exist. sotOS acknowledges the Fallacies of Distributed Computing.
...
sotOS v0.1.0 -- microkernel booting...
[ok] GDT
[ok] IDT
...
\toprule
\textbf{Property} & \textbf{Legacy UNIX} & \textbf{seL4} & \textbf{Fuchsia} & \textbf{sotOS} \\
\midrule
...
Every service in sotOS exposes metrics and traces natively:
...
to run natively on sotOS. Unlike a hypervisor that virtualizes hardware,
LUCAS is a \textit{paravirtualizer of interfaces} --- it translates monolithic
UNIX semantics into sotOS's decentralized, capability-based primitives.

\begin{principalbox}{LUCAS in One Sentence}
LUCAS presents the illusion of a complete Linux system to legacy binaries,
while every operation is actually mediated by capabilities and sotOS IPC
...
\node[kern] (sotos) at (0, 0.5)
  {\textbf{sotOS Primitives}: IPC Endpoints \textbullet{} Channels \textbullet{}
...
\textbf{Capability Mapper} &
  ...
  to real sotOS capabilities. Emulates UID/GID permissions on top of the
...
\textbf{IPC/Network Translator} &
  ...
  Emulates \texttt{epoll}/\texttt{select} over sotOS IPC. \\
\bottomrule
\end{tabularx}
\end{center}
...
In sotOS, there is \textbf{no ambient authority}. A Linux binary running inside LUCAS
...
\begin{itemize}[leftmargin=1.5em]
  \item The compromised binary holds no native capabilities to interact with other
        sotOS protection domains or the underlying hardware.
...
{\large\sffamily\color{accent} sotOS --- The kernel guarantees isolation, not policy.}\\[0.5em]
